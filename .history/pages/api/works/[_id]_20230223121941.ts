...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === 'PUT') {
        ...
        try{
            ...
            const updateWork = await WorkModel.updateOne({'id': id})

            if(!updateWork){
                throw new Error('updateWork')
            }

            return res.status(201).json({ message: `Le projet ${workCreate.title} a bien été modifié`})
