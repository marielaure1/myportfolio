...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === 'PUT') {
        ...
        try{
            ...
            const updateWork = await WorkModel.updateOne({'id': id})await Creneaux.updateOne({dateStart : verifDateStart}, { $set : { client: userNickname, disponible: false } })

            if(!updateWork){
                throw new Error('Update Work')
            }

            return res.status(201).json({ message: `Le projet ${updateWork.title} a bien été modifié`})
